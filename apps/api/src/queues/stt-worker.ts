...
export async function runSttJob(
  ...
  await runAsSystem(async () => {
    const consult = await prisma.consultation.findUnique({
      ...
      // The patient comes along for `fixture` below — the mock STT keys its seeded demo
      // recordings on patientCode. Real providers ignore it.
      include: {
        visit: { include: { patient: { select: { patientCode: true } } } },
      },
    });
    if (!consult) throw new NotFoundError("Consultation not found");
    if (!consult.audioStorageKey)
      throw new AppError("No audio to transcribe", 422, "NO_AUDIO");
    ...
    try {
      ...
      const language =
        (["en-IN", "hi-IN", "ta-IN"] as const).find(
          (l) => l === process.env.STT_LANGUAGE,
        ) ?? "auto";
      const result = await deps.stt.transcribe(audio, {
        language,
        mimeType: "audio/webm",
        fixture: consult.visit.patient?.patientCode,
      });
      ...
      await writeWorkerAudit(prisma, clinicId, "STT_FAILED", consult.id, {
        error: message,
      });
      await deps.emit(consult.id, {
        type: "FAILED",
        data: { stage: "stt", message },
      });
      throw err;
    }
  });
